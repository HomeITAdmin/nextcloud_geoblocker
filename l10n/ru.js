OC.L10N.register(
    ...
    "ERROR: \"geoip2.phar\" does not seem to be place correctly or do not have the correct access rights." : "ОШИБКА: Кажется, что \"geoip2.phar\" размещён неправильно или у него нет правильных прав доступа.",
    ...
    "The database is currently updating, but the service can be used during the update." : "База данных в настоящее время обновляется, но сервис можно использовать во время обновления.",
    "Something is missing." : "Отсутствует необходимы компонент.",
    "No database available!" : "База данных не доступна.",
    "RIR seems to have changed the file format." : "RIR, похоже, изменил формат файла.",
    "Exception caught during Update:" : "Ошибка во время обновления:",
    "Invalid file handle. Probably the internet connection got lost during the update." : "Некорректная ссылка на файл, возможно подключение к интернету было прервано во время обновления.",
    "Database contains old version information. Reset the database using the command line tool." : "База данных содержит информацию о старой версии. Сбросьте базу данных с помощью инструмента командной строки.",
    ...
    "This is a front end to geo localization services, that allows blocking (beta), delaying (beta) and logging of login attempts from specified countries.\n\nCountries can be specified using allowlisting or blocklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Regional Internet Registries (Nextcloud SQL database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set up the localization services please have a look at the GitHub repository (homepage on the right side)." : "Это интерфейс для служб геолокации, который позволяет блокировать (бета), откладывать (бета) и регистрировать попытки входа в систему из указанных стран.\n\nСтраны можно указать с помощью списков разрешений или блокировок.\n\nВ настоящее время доступны следующие услуги по локализации:\n- Geoiplookup (локальная программа на хосте)\n- MaxMind GeoLite2 (доступ к локальной базе данных через PHP API)\n- Данные из региональных интернет-реестров (база данных Nextcloud SQL создается с данными, загруженными с FTP-серверов RIR)\n\nЧтобы получить помощь по настройке служб локализации, посетите репозиторий GitHub (домашняя страница справа).",
    "Loading" : "Загружается",
    ...
    "Login attempts from local network IP addresses are not reacted on at all." : "Попытки входа в систему с IP-адресов локальной сети не принимаются.",
