OC.L10N.register(
    ...
    "The user \"%s\" attempt to login with an invalid IP address \"%s\"." : "Uživatel \"%s\" se pokouší o přihlášení s neplatnou IP adresou \"%s\".",
    "NOT_SHOWN_IN_LOG" : "NEZOBRAZENO_V_ZAZNAMU_UDALOSTI",
    ...
    "This is a front end to geo localization services, that allows blocking (beta), delaying (beta) and logging of login attempts from specified countries.\n\nCountries can be specified using allowlisting or blocklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Reginal Internet Registries (Nextcloud SQL database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set up the localization services please have a look at the GitHub repository (homepage on the right side)." : "Toto je nadstavba nad geolokačními službami, která umožňuje blokování (ve zkušebním režimu), prodlužování (dtto) a zaznamenávání pokusů o přihlašování z určených zemí.\n\nZemě je možné zadávat do seznamu povolených nebo vyloučených.\n\nSlužby, které je v tuto chvíli možné využívat jsou:\n- Geoiplookup (místní program na serveru)\n- MaxMind GeoLite2 (místní databáze, ke které se přistupuje pomocí PHP API)\n- Data z Regional Internet Registries (Nextcloud SQL databáze tvořená z dat, stažených z FTP serverů organizace RIR)\n\t\n\tNápovědu k tomu, jak nastavit služby naleznete v repozitáři na portálu Github (domovská stránka uvedená na pravé straně).",
    "Loading" : "Načítání",
    ...
    "Login attempts from local network IP addresses are not reacted on at all." : "Na pokusy o přihlášení z IP adres z místní sítě není vůbec reagováno.",
    ...
    "(Delaying is enforced for 30s before password check.)" : "(Je vynucena 30s prodleva před kontrolou hesla.)",
    ...
    "(Blocking is enforced after password check!)" : "(Blokování je vynuceno po kontrole hesla!)",
    "Test" : "Vyzkoušet",
