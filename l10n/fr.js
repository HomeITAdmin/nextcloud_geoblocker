OC.L10N.register(
    ...
    "This is a front end to geo localization services, that allows blocking (currently only logging is possible!) of login attempts from specified countries. (Early Version)\n\nCountries can be specified using whitelisting or blacklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Reginal Internet Registries (Nextcloud SQL Database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set the services up please have a look at the Github repository (homepage on the right side)." : "Il s'agit d'une application cliente pour des services de géolocalisation permettant le blocage de tentatives d'authentification provenant de pays définis (version préliminaire, seule la journalisation fonctionne).\n\nLes pays peuvent être configurés à l'aide de listes blanches ou de listes noires.\n\nLes services de localisation actuellement disponibles sont :\n - Geoiplookup (programme exécuté localement sur le serveur)\n - MaxMind GeoLite2 (base de données locale accessible depuis une API PHP)\n - Données provenant de Registres Internet Régionaux (Base de données SQL Nextcloud construite avec des données téléchargées des serveurs FTP des RIR)\n\nL'aide pour installer les services est disponible dans le dépôt Github (page d'accueil sur le côté droit).",
    "Loading" : "Chargement",
