OC.L10N.register(
    ...
    "This is a front end to geo localization services, that allows blocking (beta), delaying (beta) and logging of login attempts from specified countries.\n\nCountries can be specified using allowlisting or blocklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Reginal Internet Registries (Nextcloud SQL database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set up the localization services please have a look at the GitHub repository (homepage on the right side)." : "Il s'agit d'une interface pour les services de géolocalisation, qui permet de bloquer (bêta), de retarder (bêta) et de consigner les tentatives de connexion à partir de pays spécifiés.\n\nLes pays peuvent être spécifiés à l'aide de la liste d'autorisation ou de la liste de blocage.\n\n Les services de localisation actuellement disponibles sont:\n - Geoiplookup (programme local sur l'hôte) \n- MaxMind GeoLite2 (base de données locale accessible via l'API PHP) \n- Données des Reginal Internet Registries (base de données Nextcloud SQL constituée de données téléchargées depuis les serveurs FTP RIR) \n\nPour obtenir de l'aide sur la configuration des services de localisation, veuillez consulter le référentiel GitHub (page d'accueil sur le côté droit).",
    "Loading" : "Chargement",
    ...
    "Login attempts from local network IP addresses are not reacted on at all." : "Les tentatives de connexion à partir des adresses IP du réseau local ne réagissent pas du tout.",
    ...
    "(Delaying is enforced for 30s before password check.)" : "(Le délai est appliqué pendant 30 secondes avant la vérification du mot de passe.)",
    ...
    "(Blocking is enforced after password check!)" : "(Le blocage est appliqué après la vérification du mot de passe!)",
    "Test" : "Test",
