OC.L10N.register(
    ...
    "This is a front end to geo localization services, that allows blocking (currently only logging is possible!) of login attempts from specified countries. (Early Version)\n\nCountries can be specified using whitelisting or blacklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Reginal Internet Registries (Nextcloud SQL Database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set up the localization services please have a look at the Github repository (homepage on the right side)." : "Toto je služba geo lokalizácie, ktorá umožňuje blokovanie (momentálne je možné len protokolovanie!) pokusov o prihlásenie z určitých krajín. (Skorá verzia)\n\t\nKrajiny je možné zadať do bielej alebo čiernej listiny.\n\t\n\tV súčasnosti dostupné lokalizačné služby sú:\n\t- Geoiplookup (lokálny program na hostiteľovi)\n\t- MaxMind GeoLite2 (lokálna databáza prístupná pomocou PHP API)\n\t- Údaje z regulárnych internetových registrov (Nextcloud SQL databáza  s údajmi stiahnutými z FTP serverov RIR).\n\t\nPri nastavovaní služieb vám pomôže pozrieť sa do úložiska Github (domovská stránka na pravej strane).",
    "Loading" : "Načítava sa...",
