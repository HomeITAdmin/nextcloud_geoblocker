OC.L10N.register(
    "geoblocker",
    {
    "The user \"%s\" logged in with IP address \"%s\" from blocked country \"%s\"." : "用户 “%s” 使用 IP 地址 “%s” - “%s” 登录，这是一个被封锁的国家。",
    "The login of user \"%s\" with IP address \"%s\" could not be checked due to problems with location service." : "由于位置服务问题，无法检查 IP 地址为 “%s” 的用户 “%s”的登录。",
    ...
    "Wrong Nextcloud configuration (especially in container) can lead to all access seems to come from local network IP address." : "错误的Nextcloud配置（尤其在容器中）会导致所有访问都像是来自于本地网络的IP地址。",
    "If you are accessing from external network, this should be an external address: " : "如果您是从外部网络访问，这应该是一个外网地址：",
    ...
    "Blacklisting" : "黑名单",
    "Whitelisting" : "白名单",
    "the following entries" : "以下条目",
    "The following countries were chosen: " : "以下国家被选中：",
    "Reaction" : "响应",
    "If a login attempt is detected from the blacklisted or not whitelisted countries, the attempted is logged with the following information (be aware of data protection issues)" : "如果一个登录尝试被检测到来自黑名单中或不在白名单中的国家，此次尝试由以下信息记录（请注意数据保护问题）",
    ...
    "Activate blocking of the login attempt from IPs of the specified countries." : "激活对来自指定国家的IP的登录尝试的封锁。",
