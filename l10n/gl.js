OC.L10N.register(
    ...
    "The database is currently updating, but the service can be used during the update." : "A base de datos está a actualizarse actualmente, mais o servizo pode usarse durante a actualización.",
    ...
    "Database contains old version information. Reset the database using the command line tool." : "A base de datos contén información de versión antiga. Restabeleza a base de datos usando a ferramenta de liña de ordes.",
    ...
    "This is a front end to geo localization services, that allows blocking (beta), delaying (beta) and logging of login attempts from specified countries.\n\nCountries can be specified using allowlisting or blocklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Regional Internet Registries (Nextcloud SQL database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set up the localization services please have a look at the GitHub repository (homepage on the right side)." : "Isto é unha interface para os servizos de localización xeográfica, que permite o bloqueo (beta), o atraso (beta) e o rexistro dos intentos de inicio de sesión de países especificados.\n\nOs países pódense especificar mediante listas de permisos ou listas de bloqueos.\n\nOs servizos de localización dispoñíbeis actualmente son:\n- Geoiplookup (programa local no host)\n- MaxMind GeoLite2 (base de datos local á que se accede mediante a API de PHP)\n- Datos dos rexistros rexionais de Internet (a base de datos SQL Nextcloud constrúese cos datos descargados dos servidores FTP dos RIR)\n\nPara obter axuda sobre como configurar os servizos de localización, bótelle unha ollada ao repositorio GitHub (páxina de inicio á dereita).",
    "Loading" : "Cargando",
    ...
    "(Blocking is enforced before password check by throwing an \"login denied\" exception!)" : "(O bloqueo aplícase antes da comprobación do contrasinal lanzando unha excepción de «acceso denegado»).",
    "Test" : "Proba",
