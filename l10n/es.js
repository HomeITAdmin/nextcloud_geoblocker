OC.L10N.register(
    ...
    "This is a front end to geo localization services, that allows blocking (currently only logging is possible!) of login attempts from specified countries. (Early Version)\n\nCountries can be specified using whitelisting or blacklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Reginal Internet Registries (Nextcloud SQL Database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set the services up please have a look at the Github repository (homepage on the right side)." : "Este es un front-end para los servicios de localización geográfica, que permite bloquear (¡aunque actualmente sólo es posible registrar!) los intentos de inicio de sesión desde países específicos. (Versión temprana)\n\nLos países pueden ser especificados usando listas blancas o listas negras.\n\nLos servicios de localización actualmente disponibles son:\n- Geoiplookup (programa local en el host)\n- MaxMind GeoLite2 (base de datos local accesible vía API PHP)\n- Datos desde Reginal Internet Registries (Nextcloud SQL Database creada con datos descargados desde los servidores FTP de RIR)\n\nPara ayuda acerca de cómo configurar los servicios, por favor eche un vistazo al repositorio Github (página principal indicada a la derecha).",
    "Loading" : "Cargando",
