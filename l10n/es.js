OC.L10N.register(
    ...
    "The database is currently updating. Please wait until update is finished. This may take several minutes." : "La base de datos se está actualizando. Por favor, espere hasta que la actualización termine. Esto podría tomar varios minutos.",
    ...
    "This is a front end to geo localization services, that allows blocking (currently only logging is possible!) of login attempts from specified countries. (Early Version)\n\nCountries can be specified using allowlisting or blocklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Reginal Internet Registries (Nextcloud SQL database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set up the localization services please have a look at the GitHub repository (homepage on the right side)." : "Es una interfaz de servicios de geolocalización, que permite bloquear (¡pero actualmente sólo registrar!) los intentos de acceso desde países específicos. (Versión en desarrollo)\n\nLos países pueden especificarse utilizando la lista de permitidos o la de bloqueados.\n\nLos servicios de localización disponibles actualmente son\n- Geoiplookup (programa local en el host)\n- MaxMind GeoLite2 (base de datos local a la que se accede a través de la API de PHP)\n- Datos de los Registros Regionales de Internet (la base de datos SQL de Nextcloud se construye con los datos descargados de los servidores FTP de los RIR)\n\nPara obtener ayuda sobre cómo configurar los servicios de localización, por favor, eche un vistazo al repositorio de GitHub (página de inicio en el lado derecho).",
    "Loading" : "Cargando",
    ...
    "In addition, the login attempt can also be blocked" : "Además, el intento de inicio de sesión también puede ser bloqueado",
