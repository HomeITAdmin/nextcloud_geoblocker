OC.L10N.register(
    ...
    "ERROR: Invalid Argument." : "ERROR: Argumento Invalido",
    "ERROR: \"geoip2.phar\" does not seem to be place correctly or do not have the correct access rights." : "ERROR: \"geoip2.phar\" no parece estar colocado correctamente o no tiene los derechos de acceso correctos.",
    ...
    "The database is currently updating, but the service can be used during the update." : "La base de datos se está actualizando, pero el servicio podrá ser usado durante el proceso.",
    ...
    "This is a front end to geo localization services, that allows blocking (beta), delaying (beta) and logging of login attempts from specified countries.\n\nCountries can be specified using allowlisting or blocklisting.\n\nCurrently available localization services are:\n- Geoiplookup (local programm on the host)\n- MaxMind GeoLite2 (local database accessed via PHP API)\n- Data from Regional Internet Registries (Nextcloud SQL database build up with data downloaded from the RIRs FTP servers)\n\nFor help how to set up the localization services please have a look at the GitHub repository (homepage on the right side)." : "Esto es un frontend para servicios de geolocalización que permite bloquear (beta), retrasar (beta) y evitar las entradas intentadas desde ciertos países.\n\nSe puede especifiar los países usando listas de permisos o de bloqueo.\n\nLos servicios de localización disponibles ahora mismo son:\n- Geoiplookup (programa local en el servidor).\n- MaxMind GeoLite2 (base de datos loca accedida vía una API PHP)\n- Datos de Regional Internet Registries (base de datos SQ de Nextcloud construida con datos descargados desde los servidores FTP de RIR).\n\nPara ayuda sobre cómo configurar los servicios de localización, por favor, echa un vistazo al repositorio de GitHub (página de inicio en el lado derecho).",
    "Loading" : "Cargando",
